...
void detect_ball (const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
...
int main(int argc, char **argv) {
    ...
    pcl_pub = nh.advertise<PCLCloud>("Ball", 100);
	plane_pub = nh.advertise<PCLCloud>("plane", 100);
	ros::Subscriber pcl_sub = nh.subscribe("filtered_pcl", 100, detect_ball);
	ros::spin();
	return 0;
}